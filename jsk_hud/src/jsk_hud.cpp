...
void cb_lapoint (const geometry_msgs::PoseStamped &data_in)             {status[4].ts = ros::Time::now();}
...
void cb_ref     (const autoware_msgs::ControlCommand &data_in)          {status[8].ts = ros::Time::now();
